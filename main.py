...
class WumpusGame:
    ...
    def __init__(self, mode="manual", w=8, h=8, n_pits=6, n_wumpus=3, n_arrows=1, seed=123, wumpus_orientation="down"):
        ...
        self.mode = mode
        self.CELL = 80  # Increased from 64 to 80 for ~480p (8*80=640 width)
    ...
    def handle_manual_input(self, event):
        ...
        # shooting keymap (I,J,K,L)
        shoot_keymap = {
            pygame.K_i: 4,  # shoot UP
            pygame.K_k: 5,  # shoot DOWN
            pygame.K_j: 6,  # shoot LEFT
            pygame.K_l: 7   # shoot RIGHT
        }
        ...
        if event.key in keymap:
            ...
            # step handles shooting and records path in env.last_shot_path for animation
            reward, done = self.env.step(action)
            # animate arrow if path provided
            ...
            print(f"Action: SHOOT {['UP','DOWN','LEFT','RIGHT'][action-4]}, Reward: {reward}, "
                  f"Arrows left: {self.env.arrows_remaining}, Total: {self.env.total}")
    ...
    def handle_agent_step(self):
        ...
        if action is not None:
            # allow agents to return either move (0-3) or shoot (4-7)
            reward, done = self.env.step(action)
            ...
            label = action_names[action] if 0 <= action < len(action_names) else str(action)
            print(f"Action: {label}, Reward: {reward}, "
                  f"Position: {self.env.agent}, Total: {self.env.total}")
    ...
    def animate_arrow(self, path, dir_index):
        """Animate arrow sprite over tile-by-tile path. `path` is list of (r,c)."""
        if not path:
            return
        ...
        # rotation mapping: arrow image assumed pointing RIGHT by default
        angle_map = {0: 270, 1: 90, 2: 180, 3: 0}  # up, down, left, right -> rotate degrees (pygame rotates CCW)
        # mapping of dir_index to angle_map index: our dir_index uses 0=up,1=down,2=left,3=right (action-4)
        rot = angle_map.get(dir_index, 0)
        aimg = pygame.transform.rotate(arrow_img, rot)

        # draw arrow tile-by-tile
        delay_ms = 100
        for pos in path:
            # render env background
            self.env.render(self.screen, self.CELL, self.assets)
            # blit arrow centered in tile
            x = pos[1] * self.CELL + 2
...
def main():
    ...
    parser.add_argument("--arrows", type=int, default=1, help="Number of arrows")
